// top_pin_id.v — active pin scanner with 8bpp framebuffer + TRNG dithered OLED
// Restored from pre-calculator commit f5a38bd, separate from top_pin_scan.v
// (which is the dozenal calculator).
//
...
//
// Display layout:
//   Rows 0-15:  Pin state with grey markers on suspect cols (37/38/39/52/53/54/59)
//   Rows 16-31: Live pin state (clean, no markers)
//   Rows 32-33: 2px scan cursor with 8-column fade trail
//   Rows 34-49: 7-bit binary ruler (2px per bit) — vertical readout of column
//               index at each column. Touch a header pin to GND → cell darkens
//               → read ruler vertically at that column to get pin index in binary.
//   Rows 50-63: blank
//
// Masked pins (always tristate):
//   p41 (M3) — clock/oscillator
//   p61 (R6) — board kill (driving LOW crashes the FPGA)
...
module top_pin_id #(
    ...
    wire scan_tick = (scan_timer == 20'd99999);   // 25MHz / 100000 = 250 Hz (4ms per pin) — 8× faster
